...
const currentTopMenuSize = ref(0) // Üst menü boyutu için ref
...
<template>
    <div class="bh-p-10">
        <!-- Debug bilgileri -->
        <div class="bh-p-2 bh-bg-gray-100 bh-mb-2">
            <p>Top Menü Yüksekliği (px): {{ currentTopMenuSize }}</p>
        </div>
        <div class="bh-mb-2">
        ...
        <vue3-datatable ref="datatable" :loading="false" :rows="rows" :footerRows="footer_rows" :columns="cols"
            ...
            <template #tableleftmenu>
                <div
                    style="min-width: 50px; max-width: 250px !important; width: 100%; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; box-sizing: border-box; background-color: #f5f5f5; padding: 10px;">
                    <div style="font-weight: bold; font-size: 18px; margin-bottom: 10px;">Sabit Genişlikli Menü</div>
                    <div style="margin-bottom: 5px; padding: 8px; background-color: #e0e0ff; border-radius: 4px;">Menü
                        Öğesi 1</div>
                    <div style="margin-bottom: 5px; padding: 8px; background-color: #e0e0ff; border-radius: 4px;">Menü
                        Öğesi 2</div>
                    <div style="margin-bottom: 5px; padding: 8px; background-color: #e0e0ff; border-radius: 4px;">Menü
                        Öğesi 3</div>
                    <div style="margin-bottom: 5px; padding: 8px; background-color: #e0e0ff; border-radius: 4px;">Çok
                        Uzun Bir Menü Öğesi İsmi</div>
                    <div style="margin-bottom: 5px; padding: 8px; background-color: #e0e0ff; border-radius: 4px;">Menü
                        Öğesi 5</div>
                </div>
            </template>
                ...
                <div class="bh-pagination-info bh-flex bh-items-center">
                    ...
                    <select v-if="showPageSize" @change="setPageSize($event.target.value)" class="bh-pagesize">
                        <option v-for="option in pageSizeOptions" :value="option"
                            :selected="currentPageSize == option ? true : false" :key="option">
                            {{ option }}
                        </option>
                    </select>
                </div>
            ...
            <template #footerpagination="{
                ...
                <div class="bh-pagination-number sm:bh-ml-auto bh-inline-flex bh-items-center bh-space-x-1">
                    <button type="button" class="bh-page-item first-page" :class="{ disabled: currentPage <= 1 }"
                        @click="movePage(1)">
                    ...
                    <button type="button" class="bh-page-item last-page" :class="{ disabled: currentPage >= maxPage }"
